<form id="{{request()->is('admin/adreamer/adreamer-products/create') ? 'adreamer_product_create_form' : 'adreamer_product_edit_form'}}">
    @csrf
    <div class="row">
        <div class="col-lg-4 col-md-6 col-sm-12 col-12">
            <input type="hidden" name="product_id" id="product_id" value="{{isset($adreamerProduct) ? $adreamerProduct->id : null}}">
            <div class="form-group mb-4">
                <label for="">Name <span class="text-danger">*</span></label>
                <input type="text" name="name" class="form-control name" value="{{ old('name', isset($adreamerProduct) ? $adreamerProduct->name : null) }}">
            ...
            <div class="form-group mb-4">
                ...
                <select name="category_id" id="" class="form-select select2 category_id" data-placeholder="---Please Select---">
                    ...
                    @foreach ($categories as $category)
                        <option value="{{$category->id}}" {{isset($adreamerProduct) && $adreamerProduct->adreamer_category_id == $category->id ? 'selected' : ''}}>{{$category->name}}</option>
                    @endforeach
                </select>
    ...
    <div class="border rounded p-3">
                ...
                <div class="form-group mb-4">
                    ...
                    <select name="product_type_id" id=""  class="form-select select2 product_type_id" data-placeholder="---Please Select---">
                        ...
                        @foreach ($productTypes as $type)
                            <option value="{{$type->id}}" {{isset($adreamerProduct) ? ($adreamerProduct->product_type_id == $type->id ? 'selected' : '') : ($type->id == 1 ? 'selected' : '')}}>{{$type->name}}</option>
                        @endforeach
                    </select>
        ...
        <div class="row simple-product-group {{isset($adreamerProduct) ? ($adreamerProduct->product_type_id == 1 ? '' : 'd-none') : ''}}">
            <div class="col-lg-4 col-md-6 col-sm-12 col-12">
                <div class="form-group mb-4">
                    <label for="">Quantity <span class="text-danger">*</span></label>
                    <input type="number" min="0" name="qty" class="form-control qty" value="{{ old('qty', isset($adreamerProduct) ? $adreamerProduct->qty : null) }}">
            ...
            <div class="col-lg-4 col-md-6 col-sm-12 col-12">
                <div class="form-group mb-4">
                    ...
                    <input type="number" min="0" name="price_mmk" class="form-control price_mmk" value="{{ old('price_mmk', isset($adreamerProduct) ? $adreamerProduct->price_mmk : 0) }}">
            ...
            <div class="col-lg-4 col-md-6 col-sm-12 col-12">
                <div class="form-group mb-4">
                    ...
                    <input type="number" min="0" name="price_us" class="form-control price_us" value="{{ old('price_us', isset($adreamerProduct) ? $adreamerProduct->price_us : 0) }}">
                    <span class="text-danger price_us_err"></span>
                </div>
            </div>
        </div>

        <div class="d-flex attribute-product-group align-items-start mt-4 {{isset($adreamerProduct) ? ($adreamerProduct->product_type_id == 2 ? '' : 'd-none') : 'd-none'}}">
            ...
            <div class="tab-content p-0 m-0 ps-4 w-100" id="v-pills-tabContent">
                        ...
                        <div class="col-12 col-md-6">
                            <select name="attributes[]" id="attributes" class="select2 form-control attributes " style="width: 400px;" multiple="multiple" data-placeholder="---Please Select---">
                                @foreach ($attributes as $attribute)
                                    <option value="{{ $attribute->id }}" {{isset($adreamerProduct) && $adreamerProduct->attributes->contains($attribute->id) ? 'selected' : ''}}>{{ $attribute->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="tab-pane fade" id="v-pills-variation" role="tabpanel" aria-labelledby="v-pills-variation-tab" tabindex="0">
                    ...
                    <input type="hidden" id="variation_count" value="{{isset($adreamerProduct) && $adreamerProduct->variations->count() > 0 ? $adreamerProduct->variations->count() : 0}}">
                    <div class="variation-group">
                        @if(isset($adreamerProduct) && $adreamerProduct->variations->count() > 0)
                            @foreach ($adreamerProduct->variations as $vIndex => $variation)
                                @php
    ...
    <div class="row mt-4">
        <div class="col-12">
            <div class="form-group mb-4">
                ...
                <textarea name="specification" id="" cols="30" rows="5" class="form-control specification" placeholder="Write specification ...">{{isset($adreamerProduct) ? $adreamerProduct->specification : ''}}</textarea>
        ...
        <div class="col-12">
            <div class="form-group mb-4">
                ...
                <textarea name="description" class="form-control" id="description" placeholder="Write description ..." cols="30" rows="8">{{isset($adreamerProduct) ? $adreamerProduct->description : ''}}</textarea>
            </div>
        </div>
    </div>
    <div class="mt-5">
