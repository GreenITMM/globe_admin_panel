$(document).ready(function() {
    ...
    const table = new DataTable('#DataTable', {
        ...
        responsive: true,
        serverSide: true,
        ajax: '/admin/content-management/slider-list',
        columns: [{
                data: 'plus-icon',
                name: 'plus-icon'
            },
     ...
     //submit create form
     $(document).on('submit', '#slider_create_form', function(e) {
        e.preventDefault();
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                let formData = new FormData($('#slider_create_form')[0]);

                $.ajax({
                    url: "/admin/content-management/slider",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (res) {
                        console.log(res);
                        if (res == "success") {
                            window.location.href = "/admin/content-management/slider";
                        }
                    },
    ...
    $(document).on('submit', '#slider_edit_form', function(e) {
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                let formData = new FormData($('#slider_edit_form')[0]);
                let id = $('#slider_id').val();

                $.ajax({
                    url: "/admin/content-management/update-slider/"+id,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
    ...
    $(document).on('click', '.delete-btn', function(e) {
        ...
        if(id) {
            ask_confirm('Are you sure to delete ?').then(result => {
                if(result.isConfirmed) {
                    $.ajax({
                        url: "/admin/content-management/slider/" + id,
                        type: "DELETE",
                        headers: {
                            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr(
                                "content"
