$(document).ready(function() {
    ...
    const table = new DataTable('#DataTable', {
        ...
        responsive: true,
        serverSide: true,
        ajax: '/admin/adreamer/adreamer-product-list',
        columns: [{
                data: 'plus-icon',
                name: 'plus-icon'
            },
    ...
    $(document).on('click', '.remove-variation', function(e) {
     ...
     //submit create form
     $(document).on('submit', '#adreamer_product_create_form', function(e) {
        e.preventDefault();
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                let formData = new FormData($('#adreamer_product_create_form')[0]);

                $.ajax({
                    url: "/admin/adreamer/adreamer-products",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (res) {
                        console.log(res);
                        if (res == "success") {
                            window.location.href = "/admin/adreamer/adreamer-products";
                        }
                    },
    ...
    //submit edit form
    $(document).on('submit', '#adreamer_product_edit_form', function(e) {
        e.preventDefault();
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                let formData = new FormData($('#adreamer_product_edit_form')[0]);
                let id = $('#product_id').val();

                $.ajax({
                    url: "/admin/adreamer/update-adreamer-product/"+id,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
    ...
    $(document).on('click', '.delete-btn', function(e) {
        ...
        if(id) {
            ask_confirm('Are you sure to delete ?', 'Yes, Delete').then(result => {
                if(result.isConfirmed) {
                    $.ajax({
                        url: "/admin/adreamer/adreamer-products/" + id,
                        type: "DELETE",
                        headers: {
                            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr(
                                "content"
