$(document).ready(function() {
    ...
    const table = new DataTable('#DataTable', {
        ...
        responsive: true,
        serverSide: true,
        ajax: '/admin/adreamer/adreamer-attribute-list',
        columns: [{
                data: 'plus-icon',
                name: 'plus-icon'
            },
    ...
    $(document).on('click', '.remove-attr-balue', function() {
     ...
     //submit create form
     $(document).on('submit', '#adreamer_attribute_create_form', function(e) {
        e.preventDefault();
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                let formData = new FormData($('#adreamer_attribute_create_form')[0]);

                $.ajax({
                    url: "/admin/adreamer/adreamer-attributes",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (res) {
                        console.log(res);
                        if (res == "success") {
                            window.location.href = "/admin/adreamer/adreamer-attributes";
                        }
                    },
    ...
    //submit edit form
    $(document).on('submit', '#adreamer_attribute_edit_form', function(e) {
        e.preventDefault();
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                let formData = new FormData($('#adreamer_attribute_edit_form')[0]);
                let id = $('#adreamer_attribute_id').val();

                $.ajax({
                    url: "/admin/adreamer/update-adreamer-attribute/"+ id,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
    ...
    $(document).on('click', '.delete-btn', function(e) {
        ...
        if(id) {
            ask_confirm('Are you sure to delete ?').then(result => {
                if(result.isConfirmed) {
                    $.ajax({
                        url: "/admin/adreamer/adreamer-attributes/" + id,
                        type: "DELETE",
                        headers: {
                            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr(
                                "content"
