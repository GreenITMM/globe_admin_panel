$(document).ready(function() {
    ...
    const table = new DataTable('#DataTable', {
        ...
        responsive: true,
        serverSide: true,
        ajax: '/admin/product-management/series-list',
        columns: [{
                data: 'plus-icon',
                name: 'plus-icon'
            },
     ...
     $(document).on('submit', '#series_create_form', function(e) {
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                ...
                $.ajax({
                    url: "/admin/product-management/series",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (res) {
                        console.log(res);
                        if (res == "success") {
                            window.location.href = "/admin/product-management/series";
                        }
                    },
    ...
    $(document).on('submit', '#series_edit_form', function(e) {
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                ...
                let slug = $('#series_slug').val();

                $.ajax({
                    url: "/admin/product-management/update-series/"+slug,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
    ...
    $(document).on('click', '.delete-btn', function(e) {
        ...
        if(slug) {
            ask_confirm('Are you sure to delete ?').then(result => {
                if(result.isConfirmed) {
                    $.ajax({
                        url: "/admin/product-management/series/" + slug,
                        type: "DELETE",
                        headers: {
                            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr(
                                "content"
