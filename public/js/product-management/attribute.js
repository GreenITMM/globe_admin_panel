$(document).ready(function() {
    ...
    const table = new DataTable('#DataTable', {
        ...
        responsive: true,
        serverSide: true,
        ajax: '/admin/product-management/attribute-list',
        columns: [{
                data: 'plus-icon',
                name: 'plus-icon'
            },
    ...
    $(document).on('click', '.remove-attr-balue', function() {
     ...
     //submit create form
     $(document).on('submit', '#attribute_create_form', function(e) {
        e.preventDefault();
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                let formData = new FormData($('#attribute_create_form')[0]);
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        console.log(res);
                        if (res == "success") {
                            window.location.href = "/admin/product-management/attributes";
                        }
                    },
    ...
    $(document).on('submit', '#attribute_edit_form', function(e) {
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                let formData = new FormData($('#attribute_edit_form')[0]);
                let id = $('#attribute_id').val();

                $.ajax({
                    url: "/admin/product-management/update-attribute/"+ id,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
