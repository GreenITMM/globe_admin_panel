$(document).ready(function() {
    ...
    const table = new DataTable('#DataTable', {
        ...
        responsive: true,
        serverSide: true,
        ajax: '/admin/product-management/category-list',
        columns: [{
                data: 'plus-icon',
                name: 'plus-icon'
            },
            ...
            if(result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        console.log(res);
                        if (res == "success") {
                            window.location.href = "/admin/product-management/categories";
                        }
                    },
    ...
    $(document).on('submit', '#category_edit_form', function(e) {
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                ...
                $.ajax({
                    url: "/admin/product-management/update-category/"+slug,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
    ...
    $(document).on('click', '.delete-btn', function(e) {
        ...
        if(slug) {
            ask_confirm('Are you sure to delete ?').then(result => {
                if(result.isConfirmed) {
                    $.ajax({
                        url: "/admin/product-management/categories/" + slug,
                        type: "DELETE",
                        headers: {
                            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr(
                                "content"
