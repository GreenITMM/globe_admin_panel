$(document).ready(function() {
    ...
    const table = new DataTable('#DataTable', {
        ...
        responsive: true,
        serverSide: true,
        ajax: '/admin/solar/solar-product-list',
        columns: [{
                data: 'plus-icon',
                name: 'plus-icon'
            },
    ...
    $(document).on('change', '.parent_category_id', function(e) {
        ...
        if(id) {
            $.ajax({
                url: '/admin/solar/get-child-category/' + id,
                success: function (res) {

                if(res.length > 0) {
                    $('.child_category_id').attr('disabled', false);

                    let option = '<option value="">---Please Select---</option>';
                    res.forEach(element => {
                        option += `<option value="${element.id}">${element.name}</option>`;
                    });
                    $('.child_category_id').html(option);
                } else {
                    $('.child_category_id').attr('disabled', true);
                    $('.child_category_id').html('<option value="">---Please Select---</option>');
                }
            }})
        }
    ...
    $(document).on('click', '.remove-variation', function(e) {
     ...
     //submit create form
     $(document).on('submit', '#solar_product_create_form', function(e) {
        e.preventDefault();
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                let formData = new FormData($('#solar_product_create_form')[0]);

                $.ajax({
                    url: "/admin/solar/solar-products",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (res) {
                        console.log(res);
                        if (res == "success") {
                            window.location.href = "/admin/solar/solar-products";
                        }
                    },
    ...
    //submit edit form
    $(document).on('submit', '#solar_product_edit_form', function(e) {
        e.preventDefault();
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                let formData = new FormData($('#solar_product_edit_form')[0]);
                let id = $('#product_id').val();

                $.ajax({
                    url: "/admin/solar/update-solar-product/"+id,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
    ...
    $(document).on('click', '.delete-btn', function(e) {
        ...
        if(id) {
            ask_confirm('Are you sure to delete ?', 'Yes, Delete').then(result => {
                if(result.isConfirmed) {
                    $.ajax({
                        url: "/admin/solar/solar-products/" + id,
                        type: "DELETE",
                        headers: {
                            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr(
                                "content"
