$(document).ready(function() {
    const table = new DataTable('#DataTable', {
        ...
        responsive: true,
        serverSide: true,
        ajax: '/admin/career-management/department-list',
        columns: [{
                data: 'plus-icon',
                name: 'plus-icon'
            },
     ...
     $(document).on('submit', '#department_create_form', function(e) {
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                ...
                $.ajax({
                    url: "/admin/career-management/departments",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (res) {
                        console.log(res);
                        if (res == "success") {
                            window.location.href = "/admin/career-management/departments";
                        }
                    },
    ...
    $(document).on('submit', '#department_edit_form', function(e) {
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                ...
                let slug = $('#department_slug').val();

                $.ajax({
                    url: "/admin/career-management/update-department/"+slug,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
    ...
    $(document).on('click', '.delete-btn', function(e) {
        ...
        if(slug) {
            ask_confirm('Are you sure to delete ?').then(result => {
                if(result.isConfirmed) {
                    $.ajax({
                        url: "/admin/career-management/departments/" + slug,
                        type: "DELETE",
                        headers: {
                            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr(
                                "content"
